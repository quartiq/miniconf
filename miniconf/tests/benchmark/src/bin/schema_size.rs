...
const fn meta_bytes(meta: Option<miniconf::Meta>) -> usize {
    match meta {
        Some(meta) => meta.items.len() * core::mem::size_of::<(&'static str, &'static str)>(),
        None => 0,
    }
}

const fn schema_bytes(schema: &miniconf::Schema) -> usize {
    let mut bytes = core::mem::size_of::<miniconf::Schema>() + meta_bytes(schema.meta);
        ...
        match internal {
            miniconf::Internal::Named(children) => {
                ...
                while index < children.len() {
                    bytes += meta_bytes(children[index].meta);
