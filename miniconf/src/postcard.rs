...
/// Deserialize and set a node value from a `postcard` flavor using a live key cursor.
...
/// Get and serialize a node value into a `postcard` flavor using a live key cursor.
