...
pub trait SerDe<S, Y>: Miniconf<Y> + Sized {
    ...
    fn iter_paths<const L: usize, P>() -> Result<iter::MiniconfIter<Self, S, L, P, Y>, IterError> {
        MiniconfIter::new()
    }
    ...
    /// # Returns
    /// A [MiniconfIter] of paths or an [IterError] if `L` is insufficient.
    fn unchecked_iter_paths<const L: usize, P>() -> MiniconfIter<Self, S, L, P, Y> {
